...
void sort_open_planning_leaf_vec(vector <tree<planning_leaf>::iterator> &open_planner_group_vec) {
...
vector<vector<int>> getPriority(vector<vector<int>> permt, int leaf_number) {
    srand(time(NULL));//设置变化的seed，时间改变，所以seed改变
    vector <vector<int>> priority;
    for(auto i = 0 ; i < leaf_number ; i++){
        priority.push_back(permt.at(rand()%permt.size()));
    }
    return priority;
}
...
//    vector <vector<int>> priority;
//    for(auto i = 0 ; i < 1 ; i++){
//        priority.push_back(permt.at(80));
//    }
//    return priority;
//}
...
int main(int argc, char **argv) {
    ...
    ROS_WARN_STREAM("按照机器人的个数,共有"<<permt.size()<<"种优先级配置.");

    // 实例化一个搜索树,并添加第一层树叶;将第一层所有树叶的迭代器存入init_pl_locs
    planning_tree test;
    vector<vector<int>> priority;

    // 第一层root使用所有的优先级序列来生成leaf,这样可以提高成功率
    // todo 可不可以改变如果搜不到再去扩充root层？
    priority = getPriority(permt, permt.size());
//    priority = getPriority(permt, 1);
    vector<tree<planning_leaf>::iterator> init_pl_locs = test.init_set_multi_robot_astar_planner(priority.size());
    ...
    int loop_count = 0;
    bool arrived = false;
    while ((ros::ok() && !arrived) && loop_count < 1) {
        ros::spinOnce();
        ROS_INFO_STREAM("回调函数已调用1次.");

        // only init once
        // 取leaf_number中优先级配置用于生成root层的leaf
        if (!init_already) {
            for (int i = 0; i < priority.size(); ++i) {
                // 每一层有priority.size()个leaf,每一个leaf中则有num_robots个规划器
                // todo 希望每次生成的leaf不要是所有的priority种类,而是随机选择几种以减少计算量
                ROS_WARN_STREAM("------------");
                ...
                //每个规划器树叶中有priority.size()个规划器,这里的i选择了一种优先级序列
                root_planning_leaf->set_planning_leaf(num_robots, priority[i]);

                for (int idx = 0; idx < num_robots; ++idx) {
                    root_planning_leaf->planners.at(priority[i][idx])->de_map_Callback(mapmsg);
                    root_planning_leaf->planners.at(priority[i][idx])->setTarget();
                    if (root_planning_leaf->planners.empty())
                        ROS_INFO_STREAM("规划器并没有添加到当前规划器组中!");
                    if (!root_planning_leaf->planners.at(priority[i][idx])->plan.poses.empty()) {
                        root_planning_leaf->add_feedback_from_path(
                                root_planning_leaf->planners.at(priority[i][idx]),
                                priority[i][idx]);
                    }
                }
        ...
        // todo 目前重新看代码看到这里
        // 将初始阶段的所有leaf的指针都放到一个vector中保存
        vector <tree<planning_leaf>::iterator> open_planning_leaf_vector;
        for (int k = 0; k < init_pl_locs.size(); ++k) {
            open_planning_leaf_vector.push_back(init_pl_locs.at(k));
        }
        ...
        tree<planning_leaf>::iterator last_planning_leaf;
        int max_iter_num = 800;
        for (int idx = 0; idx < max_iter_num; ++idx) {
            ROS_WARN_STREAM("正在搜索最优规划器组，本次为第"<< idx+1<<"次尝试.");
            //TODO: sort open_planning_leaf_vector by feedback
            sort_open_planning_leaf_vec(open_planning_leaf_vector);
            last_planning_leaf = open_planning_leaf_vector.front();

            ROS_WARN_STREAM("最优规划器组已找到，其feedback值为： " << last_planning_leaf->feedback);
            // 从feedback最小的一个pg开始扩展
            ROS_INFO_STREAM("扩展最优规划器中...");
            vector <nav_msgs::Path> nullpaths;

            // 更新随机一次优先级序列
            priority = getPriority(permt, leaf_number);
//            priority = getPriority(permt, 1);
            for (int i = 0; i < priority.size(); ++i) {

                open_planning_leaf_vector.push_back(grow_tree(last_planning_leaf, nullpaths, priority[i])); //这里的i控制了优先级序列的生成
                ROS_INFO_STREAM("按照优先级的类型需要生成"<<priority.size()<<"个规划器组，已生成第"<<i+1<<"个规划器组");
                //每长出一个sub pg，就放入open_planner_group_vec的末尾，并插入到tree

                test.tr->append_child(last_planning_leaf, open_planning_leaf_vector.back());
...
//                    nav_plans[k].publish(nullpaths[-1]); //TODO: 应该输出对当前机器人最优的路径　//TODO check 下标
                    ...
                    all_arrived &= open_planning_leaf_vector.back()->planners.at(priority[i][k])->arrived;
                }
                ...
                if (all_arrived) {
                    ...
                    for (int i = 0; i < 100; i++) {
                        (*open_planning_leaf_vector.back()).publish_path(fullpaths, open_planning_leaf_vector.back(),
                                                                         nav_plans);
                        r.sleep();
                        r.sleep();
                        r.sleep();
            ...
            ROS_WARN_STREAM(
                    "tree depth: " << idx /*test.tr->depth(last_planning_leaf)*/);
